...
class Neighbours:
    ...
    def __create_subcells(self):
        ...
        __box_length = self.System.info().char_length()
    ...
    def __find_subcell(self, position):
        ...
        subcell_id = [-1, -1, -1]
        for axis in range(3):
            subcell_id[axis] = math.floor(position[axis] / self.__subcell_length)

        if subcell_id[0] < 0 | subcell_id[1] < 0 | subcell_id[2] < 0:
            raise ValueError('subcell_id value is negative')
        else:
            m = self.__subcells_inrow
            subcell_id = subcell_id[0] + (subcell_id[1] - 1) * m + (subcell_id[2] - 1) * m ** 2

        return subcell_id

    def neighbours(self):
        """
        :return: retrieve neighbour list.
        """
